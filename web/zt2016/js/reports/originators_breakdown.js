$(function(){
	...
	$('#breakdown-table').DataTable({
		"language": {
		  "info": "_TOTAL_ originator(s)",
		  "infoFiltered": " <small>out of _MAX_ total monthly originator(s)</small>"
		},
		"dom":'<"top"i><"clear">',
		"order": [ 3, "desc" ],
		paging: false,
...
function drawtreemap () {
	...
	var data = $(mytable).map(function(index) {
		...
		if (index==0){
			...
			root:'Month',
			billedhours:cols[3].innerHTML,
			jobs:cols[4].innerHTML,
			newslides:cols[5].innerHTML,
			editedslides:cols[6].innerHTML,
			additionalhours:cols[7].innerHTML,
			}					
		}
	}).get();
	...
	.on("mousemove", function(event,d){
		  tooltip.html( "<ul class=\"list-group\"><li class=\"list-group-item active\">"+ d.data.originator.toUpperCase() +"<br/>" + d.data.client + "</li><li class=\"list-group-item\">" + d.data.billedhours + " billed hours</li><li class=\"list-group-item\">" + d.data.jobs + " jobs</li><li class=\"list-group-item\">" + d.data.newslides + " new slides</li><li class=\"list-group-item\">"+ d.data.editedslides + " edited slides</li><li class=\"list-group-item\">"+ d.data.additionalhours + " hours</li></ul>");
	...
	// add the text labels
	 svg
		...
		.data(root.leaves())
		.enter()
		.filter (function(d) { return (d.x1 - d.x0) > 100}) //filter out rectanmgles with a width under 100
		 .append("text")
		  ...
		  .text(function(d){ return d.data.originator.toUpperCase()})
		  .attr("font-size", "10px")

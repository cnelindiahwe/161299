...
// ################## duplicateentry ##################
	...
	{	
			...
			traveller.find(".formbuttons :nth-child(1)").after('<a href="#" class="duplicateentry">Duplicate Entry</a>');
		...
		// ##################  FORMS  ################## 		
			...
			// ##################  Insert current time into form  ################## 
				...
				{			
					$('.EntryClient').change( 
						function () {
						currentOrig=$(this).parents('form').find('.Originator').val();
							...
							$.ajax({
								...
								success: function(data) {
									$(this).parents('form').find('.Originator,.originfield select').remove();
									$(this).parents('form').find('.originfield').append(data);
									$(this).parents('form').find('.originfield').css("width","18em");
									$(this).parents('form').find('.Originator option[value="'+currentOrig+'"]').attr("selected","selected");
									
									//$('this').parents('.Originator').change( function () {
									//	goAddContact ();
									//})
								},
								error: function(data) {
									alert (data);
									//$('this').parents('.originfield').append(data);
									//$('this').parents('.Originator').css("width","11.5em");
									//$('this').parents('.Originator').change( function () {
									//	goAddContact ();
									//})
								}

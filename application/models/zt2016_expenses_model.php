<?php class Zt2016_expenses_model extends CI_Model {
...
function GetExpensesEntrie($options = array())
{
    ...
    $query = $this->db->get('zowexpensesentries');
...
// required values
   ...
   // Execute the query
    ...
    $this->db->update('zowexpensesentries');
...
function addexpenses($options =  array())
{
    ...
    // Execute the query
    $this->db->insert('zowexpensesentries');
...
function  DeleteExpensesEntrie($options =  array())
{
	...
	$this->db->where('id',$options['id']);
	$this->db->delete('zowexpensesentries');

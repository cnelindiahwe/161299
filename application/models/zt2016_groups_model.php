<?php class Zt2016_groups_model extends CI_Model {
	...
	function GetGroup($options = array())
	{
		...
		$qualificationArray = array('ID','GroupName','DefaultPrice','DefaultCurrency','DefaultPaymentDays','DefaultCountry','DefaultTimeZone','Active','Trash');
	...
	function AddGroup($options =  array())
	{
		...
		// qualification (make sure that we're not allowing the site to insert data that it shouldn't)
		$qualificationArray = array('GroupName','DefaultPrice','DefaultCurrency','DefaultPaymentDays','DefaultCountry','DefaultTimeZone','Active','Trash');
	...
	*/
	...
	function GetGroupContacts ($options = array())
	{
   	 	// required values
		if(!$this->_required(array('GroupName'), $options)) return false;
		
		$joinstring='zowtrakclients.CompanyName = zowtrakcontacts.CompanyName AND zowtrakclients.Trash=0 AND zowtrakclients.Group ="'.$options['GroupName'].'"';
		if(isset($options['Active'])){
			$joinstring.=' AND zowtrakcontacts.Active=1';
		}
		
		$this->db->select('*');
		$this->db->from('zowtrakcontacts');
		$this->db->join('zowtrakclients',$joinstring );
	...
	################ Retrieve group first request
	
	function GetGroupFirstLastDates ($options = array())
	{
   	 	// required values
		if(!$this->_required(array('GroupName'), $options)) return false;		
		
		
		$this->db->select('MAX(DateOut) as LastRequest, MIN(DateOut) as FirstRequest');
		$joinstring='zowtrakclients.CompanyName = zowtrakentries.Client AND zowtrakclients.Trash=0 AND zowtrakclients.Group ="'.$options['GroupName'].'"';

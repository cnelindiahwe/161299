<?php class Zt2016_client_figures_model extends CI_Model {

	
	
	################ add client
	
	function AddClientAnnualData($options =  array())
	{
		
		// qualification (make sure that we're not allowing the site to insert data that it shouldn't)
		$qualificationArray = array('ID','CompanyName','Year',"BilledHours","Jobs","Originators","NewSlides","EditedSlides","AdditionalHours");
		...
		// Execute the query
		$this->db->insert('zowtrakannualclientfigures');
	...
	function UpdateClientAnnualData($options = array())
	{
    ...
    // required values
	   ...
	   // Execute the query
		...
		$this->db->update('zowtrakannualclientfigures');
	...
	function GetClientAnnualData($options = array())
	{
		...
		$query = $this->db->get('zowtrakannualclientfigures');
	...
	function DeleteClientAnnualData($options = array())
	{
   	 	// required values
		if(!$this->_required(array('ID'), $options)) return false;
	   // Execute the query
		$this->db->where('ID', $options['ID']);
		$this->db->delete('zowtrakannualclientfigures');
		return $this->db->affected_rows();
		}

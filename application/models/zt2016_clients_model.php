<?php class Zt2016_clients_model extends CI_Model {

	################ Get client
	
	function GetClient($options = array())
	{
		...
		$qualificationArray = array('ID','CompanyName','ClientCode','Group','VATOther','Address','Address2','Address3','Address4','ZIPCode','City','Country','TimeZone',
		'Website','ZOWContact', 'ClientContact', 'BilledBy','PONumber', 'Currency','BasePrice','VolDiscount1Trigger','VolDiscount1Price',
		...
		'GroupDir','ClientGuidelines','BillingGuidelines','OtherInfo','Trash','Internal');
    ...
    // required values
		...
		$qualificationArray = array('ID','CompanyName','ClientCode','Group','VATOther','Address','Address2','Address3','Address4','ZIPCode','City','Country','TimeZone',
		'Website','ZOWContact', 'ClientContact', 'BilledBy','PONumber','BillingAddress', 'Currency','BasePrice','VolDiscount1Trigger','VolDiscount1Price',
		'VolDiscount2Trigger','VolDiscount2Price','VolDiscount3Trigger','VolDiscount3Price','VolDiscount4Trigger',
		'VolDiscount4Price','PriceEdits','RetainerHours','RetainerCycle','PaymentDueDate','OfficeVersion','CustomApps','ClientDir',
		'GroupDir','ClientGuidelines','BillingGuidelines','OtherInfo','Trash','Internal');
	...
	function AddClient($options =  array())
	{
		...
		// qualification (make sure that we're not allowing the site to insert data that it shouldn't)
		$qualificationArray = array('ID','CompanyName','ClientCode','Group','VATOther','Address','ZIPCode','City','Country','TimeZone',
		'Website','ZOWContact', 'ClientContact', 'BilledBy', 'Currency','BasePrice','VolDiscount1Trigger','VolDiscount1Price',
		'VolDiscount2Trigger','VolDiscount2Price','VolDiscount3Trigger','VolDiscount3Price','VolDiscount4Trigger',
		'VolDiscount4Price','PriceEdits','RetainerHours','RetainerCycle','PaymentDueDate','OfficeVersion','CustomApps','ClientDir',
		'GroupDir','ClientGuidelines','BillingGuidelines','OtherInfo','Trash','Internal');
	...
	function DeleteClient($options = array())
	{
   	 	// required values
		if(!$this->_required(array('ID'), $options)) return false;
	   // Execute the query
	...
	*/
	...
	// ------------------------------------------------------------------------
	/**
	* _UpdateClientsGroupName updates group field in clients table on group name change in group table
	*
	* @param array $required
	* @param array $data
	* @return bool
	*/
	...
	function _UpdateClientsGroupName($oldgroupname,$newgroupname)
	{

		$this->db->set('Group',$newgroupname);
		$this->db->where('Group', $oldgroupname);
		$this->db->update('zowtrakclients');
		return $this->db->affected_rows();

<?php class Zt2016_reports_model extends CI_Model {
	...
	*/
	...
	function _Client_Annual_Figures_data($options =  array())
	{
		if(!$this->_required(array('Client','CurrentYear'), $options)) return false;
		
		$StartDate = date( 'Y-m-d', strtotime('first day of January '.$options['CurrentYear']));
		$EndDate = date( 'Y-m-d', strtotime('last day of December '.$options['CurrentYear']));
		
		 
		$historical['Year'] = date( 'Y', strtotime( $StartDate));
		...
		$historical['EditedSlides'] = $query->row()->EditedSlides;
		$historical['Additionalhours'] = $query->row()->Hours;
		
		//if (strtotime($StartDate) < strtotime("-2 year", time())){
		if ($options['CurrentYear'] <  date("Y")-2){
			$historical['BilledHours'] = $query->row()->InvoiceTime;
			$historical['Year'] = date( 'Y', strtotime( $StartDate))."OLD";
		} 
		else {
			$historical['BilledHours'] =$historical['NewSlides']/5;
			$historical['BilledHours'] += $historical['EditedSlides']/10;
			$historical['BilledHours'] +=$historical['Additionalhours'];
		}
		...
		$historical['Originators'] = $query3 ->num_rows();
	...
	/**
	* _Originator_Annual_Figures_data
	*
	* @param array $required
	* @param array $data
	* @return bool
	*/
	...
	function _Originator_Annual_Figures_data($options =  array())
	{

		if(!$this->_required(array('OriginatorInfo','CurrentYear'), $options)) return false;
		...
		$this->db->select_sum('InvoiceTime','InvoiceTime');
		$this->db->from('zowtrakentries');
		$this->db->where('Client', $options['OriginatorInfo']->CompanyName);
		$this->db->where('Originator', $options['OriginatorInfo']->FullName);		
		$this->db->where('DateOut >=', $StartDate);
		$this->db->where('DateOut <= ', $EndDate);
		$this->db->where('Trash =',0);
		$this->db->where('Invoice !=','NOT BILLED');

...
if ( ! function_exists(' _getOngoingJobs'))
{
	...
	function  _getOngoingJobs($getentries,$ZOWuser)
			...
			foreach($getentries as $project)
			{
					...
					$entries .=" in ".$TimeZoneOut[1]." (now: ".date('H:i D, j M',strtotime($pendinghours['currenttime'])).")";
...
if ( ! function_exists(' _displayOngoingJobs'))
{
	...
	function  _displayOngoingJobs($OngoingJobsdata=0,$ZOWuser) 
	{
			...
			$entries .="</h1>";
			//Add extra buttons
			$entries .="<a href=\"trackingnew\" class=\"newjob\">Create New Job</a></h3>\n";
			//Add logout buttons
			$entries .="<a href=\"".site_url()."main/logout\" class=\"logout\">Logout</a>";
			//Add reports buttons
			$entries .="<a href=\"".site_url()."reports\" class=\"logout\">Reports</a>";
...
if ( ! function_exists(' _entrydatatable'))
{
	function _entrydatatable($getentries)
	{
		...
		foreach($getentries as $project){
			...
			$entries.="</a></td><td>";
				if ($project->EditedSlides !=0){
					$entries.=$rowlink.$project->EditedSlides;
				}
			$entries.="</a></td><td>";
				if ($project->Hours !=0){
					$entries.=$rowlink.$project->Hours;
				}
			$entries.="</a></td><td>".$rowlink.$project->Client."</a></td><td>".$rowlink.$project->Originator."</a></td><td>".$rowlink.date('D d M, Y',strtotime($project->DateOut))."</a></td><td>".$rowlink.$project->TimeOut."</a></td>";
			$entries.="<td>";

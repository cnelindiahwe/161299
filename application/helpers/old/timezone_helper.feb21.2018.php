	...
	function  TimeZoneDropDown($selected='',$id='TimeZone')
	{
		...
		foreach(timezone_abbreviations_list() as $abbr => $timezone){
		        foreach($timezone as $val){
		                if(isset($val['timezone_id']) && !in_array($val['timezone_id'],$temp_TimezonesList) && strpos($val['timezone_id'],"/")){
		                     $temp_TimezonesList []=$val['timezone_id'];
		                }
		        }
		}
		sort($temp_TimezonesList);
		
		###### (add keys as above deletes keys)
		$TimezonesList = array();
		$TimezonesList[""]="";
		foreach($temp_TimezonesList as $finalTZ){
			
			#calculate GMT offset	
			$dtz = new DateTimeZone($finalTZ);	
			$time_now = new DateTime('now', $dtz);
			$tz_offset = $dtz->getOffset( $time_now ) / 3600;
			
			
			if ($tz_offset > 0) {
				$tz_offset="GMT + ".$tz_offset;
			}
			else if  ($tz_offset == 0) {
				$tz_offset="GMT"; 	
			}
			...
			$TimezonesList[$finalTZ]=$finalTZ." (".$tz_offset.")";

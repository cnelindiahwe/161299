 ...
 * @return	string
 */
if ( !function_exists('_fetchClientMonthInvoicePrice'))
{
	function _fetchClientMonthInvoicePrice($clientdata,$bookedtotal){
 ...
 */
	...
	function _MonthPricestable($monthtotal,$StartDate){
			...
			foreach ($monthtotal as $row) {
					...
					$monthTotalTable.="</tr>";
					$hourstotal=$hourstotal+$row['Total'];
			...
			$output="<div class=\"content\"><h3 class=\"title\">".$hourstotal." hours completed in ".Date('F Y', strtotime($StartDate))." = ";
 ...
 */
 	function  _buildSplitTables($AllTotals,$StartDate,$EndDate,$Status,$ClientList,$monthtotals,$Partners)
	{
			...
			$VahishalisplitUSD=0;
			 $TotalEUR=0;
						...
						foreach($StatusType as $Status) {
								...
								if (array_key_exists($Partner,$aux)){ 
									...
									if ($Status=='ScheduledBy'){
										...
										$FD=$monthtotals[$clientb->Client]['Price']*$AllTotals[$clientb->Client][$Status][$Partner]['total'];
										...
										$WK=$monthtotals[$clientb->Client]['Price']*$AllTotals[$clientb->Client][$Status][$Partner]['total'];
										$WKcell=$WK*.4;

									}
									else if ($Status=='ProofedBy'){
										$PR=$monthtotals[$clientb->Client]['Price']*$AllTotals[$clientb->Client][$Status][$Partner]['total'];
										$PRcell=$PR*.1;
				...
				if ($Partner!=""){
					...
					if ($Partner=="Vaishali" || $Partner=="Tarun" || $Partner=="Manali" || $Partner=="Kanchan" || $Partner=="Agnel"){
						$VahishalisplitEUR=$VahishalisplitEUR+$partnertotalEUR;
						$VahishalisplitUSD=$VahishalisplitUSD+$partnertotalUSD;
						}
				}
			}
			...
			$split.= "<h3>Sunil: ".number_format($SunilsplitEUR+$VahishalisplitEUR, 2, '.', ',')." EUR | ";
			$split.= number_format($SunilsplitUSD+$VahishalisplitUSD, 2, '.', ',')." USD</h3>";
			$split.= "<p>(".number_format($SunilsplitEUR, 2, '.', ',')." + ".number_format($VahishalisplitEUR, 2, '.', ',')." EUR | ";
			
			$split.= number_format($SunilsplitUSD, 2, '.', ',')." + ".number_format($VahishalisplitUSD, 2, '.', ',');
			$split.= " USD)</p>";
			$split.= "</div>";

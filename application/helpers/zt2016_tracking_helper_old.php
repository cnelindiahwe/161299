...
if ( ! function_exists(' _Display_Ongoing_Entries_Table'))
{
	...
	function _Display_Ongoing_Entries_Table($OngoingEntries)
	{
		...
		$EntriesTable ='<table class="table table-striped table-hover table-condensed responsive dtr-inline" style="width:100%;display:none;" id="ongoing_jobs_table">'."\n";
		...
		if (!empty ($OngoingEntries['entries'])) {
			...
			foreach($OngoingEntries['entries'] as $EntryKey => $EntryDetails)
			{
				...
				##### pending
				$EntriesTable .="<td class=\"text-right\" data-order=\"".$EntryDetails['DateTimeOutTimestmap']."\" >\n";
				
				$PendingRowLink="<a href=\"".site_url()."zt2016_edit_job/".$EntryDetails['id']." \" data-toggle=\"tooltip\" title=\"".$EntryDetails['DateTimeOutTimestmap']."\" class=\"projectlink\" >";

				$EntriesTable .="<strong>".$PendingRowLink.$EntryDetails['Pending']."</a></strong>\n";
				$EntriesTable .="</td>\n";

				##### Status
				$StatusRowLink="<a href=\"".site_url()."zt2016_edit_job/".$EntryDetails['id'] . "\" class=\"projectlink btn ".$classes."\"  >";
				$EntriesTable .="<td data-order=\"".$dataorder."\">".$StatusRowLink.$EntryDetails['Status']."</a></td>\n";			
			...
			foreach($RawOngoingEntries as $project)
			{
				   ...
				   $ProjectData['Pending']=$pending; 
					...
					switch ( $project->Status){
						case 'TENTATIVE':
							$ProjectData['ZOWMember']=$project->TentativeBy;
							Break;
						case 'SCHEDULED':
							$ProjectData['ZOWMember']=$project->ScheduledBy;
							Break;
						case 'IN PROGRESS':
							$ProjectData['ZOWMember']=$project->WorkedBy;
							Break;
						case 'IN PROOFING':
							$ProjectData['ZOWMember']=$project->ProofedBy;
							Break;
					}	

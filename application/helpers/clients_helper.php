...
/**
 * _getTravellerClientMaterials()
 *
 * Provides a click-and-download list of templates and other materials for a given client for the traveller
 *
 * @access	public
 * @return	string
 */
if ( !function_exists('getTravellerClientMaterials'))
{
	function  getTravellerClientMaterials($clientcode="",$SuperUser=0)
	{
		if ($clientcode !=""){
			$clientcode=strtoupper($clientcode);
			$dir = $_SERVER['NFSN_SITE_ROOT'] . "protected/clientmaterials/".$clientcode."/";
			// Open a known directory, and proceed to read its contents
			if (is_dir($dir)) {
				$ClientMaterials = "Unit:";
				if ($SuperUser!=0) { $ClientMaterials .= "<br/><small>(Looking at ".$dir.")</small>";}	
				$ClientMaterials .= "<br/>";	
					...
					if ($num_files >0){
							if ($dh = opendir($dir)) {
									while (($file = readdir($dh)) !== false) {
											if ($file!='.' && $file!='..') {
												$filedate = date ('F d Y ', filemtime($dir.$file));
												$ClientMaterials.= '<a href="http://'.$_SERVER['SERVER_NAME'].'/zowtrak2012/clients/ajax_downloadmaterials/'.$clientcode.'/'.$file.'">'.$file."</a> <small>(".$filedate.")</small><br/>\n";
												//$ClientMaterials.= '<a href="http://'.$_SERVER['SERVER_NAME'].'/zowtrak2012/clients/deleteclientfile/'.$clientcode.'/'.$file.'" class="deletefile"><img src="'.base_url().'/web/img/trash.gif"></a>'."\n<br/>";
											}
									}
									closedir($dh);
							}
							else {$ClientMaterials.= '<div class="alert alert-warning" role="alert">No files found for client code '.$clientcode." (code 1)<br/></div>";}							
					}
					else {$ClientMaterials.= '<div class="alert alert-warning" role="alert">No files found for client code '.$clientcode." (code 2)<br/></div>";}
			}
			else {$ClientMaterials= '<div class="alert alert-danger" role="alert">ERROR - No client directory found.<br/></div>';}
		}
		else {$ClientMaterials= '<div class="alert alert-danger" role="alert">ERROR - No client code provided.<br/></div>';}
...
/**
 * _getTravellerClientGroupMaterials()
 *
 * Provides a click-and-download list of templates and other materials for a given client group for the traveller
 *
 * @access	public
 * @return	string
 */
if ( !function_exists('getTravellerClientGroupMaterials'))
{
	function  getTravellerClientGroupMaterials($clientcode="",$SuperUser=0)
	{
		if ($clientcode !=""){
			$clientcode=strtoupper($clientcode);
			...
			$CI =& get_instance();
			$CI->load->model('trakclients', '', TRUE);
			$getclientdata = $CI->trakclients->GetEntry($options = array('ClientCode' => $clientcode));
			if ($getclientdata) {
				if ($getclientdata->GroupDir!="") {
					$dir = $_SERVER['NFSN_SITE_ROOT'] . "protected/clientmaterials/group/".$getclientdata->GroupDir."/";
					// Open a known directory, and proceed to read its contents
					if (is_dir($dir)) {
						$ClientMaterials = "Group:";
						if ($SuperUser!=0) { $ClientMaterials .= "<br/><small>(Looking at ".$dir.")</small>";}	
						$ClientMaterials .= "<br/>";	
							...
							if ($num_files >0){
									if ($dh = opendir($dir)) {
											while (($file = readdir($dh)) !== false) {
													if ($file!='.' && $file!='..') {
														$filedate = date ('F d Y ', filemtime($dir.$file));
														$ClientMaterials.= '<a href="http://'.$_SERVER['SERVER_NAME'].'/zowtrak2012/clients/ajax_downloadmaterials/group/'.$getclientdata->GroupDir.'/'.$file.'">'.$file."</a> <small>(".$filedate.")</small><br/>\n";
														//$ClientMaterials.= '<a href="http://'.$_SERVER['SERVER_NAME'].'/zowtrak2012/clients/deleteclientfile/'.$clientcode.'/'.$file.'" class="deletefile"><img src="'.base_url().'/web/img/trash.gif"></a>'."\n<br/>";
													}
											}
											closedir($dh);
									}
									else {$ClientMaterials.= '<div class="alert alert-warning" role="alert">No files found for group code '.$clientcode." (code 1)<br/></div>";}							
							}
							else {$ClientMaterials.= '<div class="alert alert-warning" role="alert">No files found for group code '.$clientcode." (code 2)<br/></div>";}
					}
					else {$ClientMaterials= '<div class="alert alert-danger" role="alert">This client does not belong to a group<br/></div>';}
				}
				else { $ClientMaterials= '<div class="alert alert-danger" role="alert">This client does not belong to a group<br/></div>';}
			}
			else { $ClientMaterials= '<div class="alert alert-danger" role="alert">ERROR retrieving client code'.$clientcode.'<br/></div>';}
		}
		else {$ClientMaterials= '<div class="alert alert-danger" role="alert">ERROR - No client code provided.<br/></div>';}
...
if ( !function_exists('zt2016_getClientMaterials'))
{
	function  zt2016_getClientMaterials($dirname="",$superuser)
	{
		if ($dirname !=""){
			...
			$clientcode=$dir[0];
			$dir = $_SERVER['DOCUMENT_ROOT']."/zowtempa/etc/clientmaterials/".$finaldir."/";
			// Open a known directory, and proceed to read its contents
			if (is_dir($dir)) {

...
if ( ! function_exists('_Set_Past_Entries_Calendar_Prefs'))
{
	...
	function  _Set_Past_Entries_Calendar_Prefs($suffix,$navtargeturl="zt2016_pastjobs",$calendaryear,$calendarmonth)
	{
		
		
		$navurl=site_url().'tracking/'.$navtargeturl.'/calendarview/';
		$dateurl=site_url().'tracking/'.$navtargeturl.'/dateview/';
		...
		/*$datelinkURL = "<a href=\"".site_url().'tracking/zt2016_past_jobs/dateview/'.date_format(new DateTime ($row['DateOut']), 'Y')."/".date_format(new DateTime ($row['DateOut']), 'M')."/".date_format(new DateTime ($row['DateOut']), 'd').'">';*/		
		...
		$prefs['template'] = '
		   ...
		   {heading_previous_cell}<th style="width:15%;"><a href="{previous_url}'.$suffix.'" class="monthviewnav">&lt;&lt;</a></th>{/heading_previous_cell}
		   {heading_title_cell}<th colspan="{colspan}" style="text-align:center;">{heading}</th>{/heading_title_cell}
		   {heading_next_cell}<th style="width:15%;"><a href="{next_url}'.$suffix.'"  class="monthviewnav pull-right">&gt;&gt;</a></th>{/heading_next_cell}
		   ...
		   {cal_cell_content}<div class="CalendarDay"><a href="'.$dateurl.$calendaryear.'/'.$calendarmonth.'/{day}"><strong>{day}</strong></a></div><div><ul>{content}</ul></div>{/cal_cell_content}
		   {cal_cell_content_today}<div class="CalendarDay"><a href="'.$dateurl.$calendaryear.'/'.$calendarmonth.'/{day}"><strong>{day}</strong></a></div><div><ul>{content}</ul></div>{/cal_cell_content_today}

		   {cal_cell_no_content}<div class="CalendarDay"><a href="'.$dateurl.$calendaryear.'/'.$calendarmonth.'/{day}">{day}</a></div>{/cal_cell_no_content}
		   {cal_cell_no_content_today}<div class="CalendarDay"><a href="'.$dateurl.$calendaryear.'/'.$calendarmonth.'/{day}">{day}</a></div>{/cal_cell_no_content_today} 

...
class Newinvoice extends MY_Controller {
	...
	function index()
	{
		...
		$invoicedata=getInvoiceByDate($client,$StartDate,$EndDate);
		...
		if ($templateVars['pageOutput']!="No entries since last invoice.\n") {
			$templateVars['pageOutput']=$this-> _getnewinvoicetop($clientobject,$StartDate,$EndDate,$invoicedata).$templateVars['pageOutput'];
		}
		else {
	...
	// ################## top ##################	
	function  _getnewinvoicetop($clientobject,$StartDate="",$EndDate="",$invoicedata="")
	{
			$entries ="<div id='newjobbuttons' class='zowtrakui-topbar'>\n";
			$entries .="<h1>New Invoice for ".$clientobject->CompanyName."</h1>";
			if ($StartDate!="") {
				$entries .=$this->_NewInvoiceForm($clientobject,$StartDate,$EndDate,$invoicedata);
			}
			else{
	...
	// ################## New Invoice Form ##################	
	function  _NewInvoiceForm($clientobject,$StartDate,$EndDate,$invoicedata)
	{
		...
		{ 
			$InvoiceCount+=1;
			if ($InvoiceCount<10) {
				$TestInvoiceNumber = $InvoiceNumber.'0'.$InvoiceCount.date("Ym");
			}
			else{
				$TestInvoiceNumber = $InvoiceNumber.$InvoiceCount.date("Ym");
			}

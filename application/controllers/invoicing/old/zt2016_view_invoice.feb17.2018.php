...
class Zt2016_view_invoice extends MY_Controller {
	...
	function  _getinvoiceContent($invoicenumber)
	{
		...
		# call helper
		$pageOutput = $this->zt2016_display_existing_invoice($invoiceTotals,$clientInfo,$contactInfoTable);
	...
	function  zt2016_display_existing_invoice($invoiceTotals,$clientInfo,$contactInfoTable)
	{
		...
		$pageOutput.='<li role="presentation" class="active"><a href="#invoice-main" aria-controls="invoice-main" role="tab" data-toggle="tab">Invoice</a></li>'."\n";
			...
			$Originators= explode(",", $invoiceTotals->Originators);
			foreach ($Originators as $Originator){
				...
				foreach ($contactInfoTable  as $Contact){ 
					...
					if ($Originator==$ContactFullName){
						if (!empty($Contact->ContactBillingGuidelines)){
							$notesCount++;
		...
		$pageOutput.= "<pre class='pre-scrollable' id='originators_list'>".$invoiceTotals->Originators."</pre>";
 		...
 		############  Invoice data
		$pageOutput.= zt2016_getInvoiceTableByNumber($invoiceTotals->InvoiceNumber,$clientInfo,$contactInfoTable);
  		...
  		$pageOutput.='	<div role="tabpanel" class="tab-pane" id="invoice-notes">'."\n";
			...
			$pageOutput.='		<div class="col-md-4">'."\n";
				...
				foreach ($Originators as $Originator){
					...
					if (count($Originators)<6){
						
							$attributes='class="contact-billing-guidelines-form"';
							$formurl=site_url().'contacts/zt2016_contact_billing_info_update/';
							$pageOutput.=form_open($formurl,$attributes )."\n";
							$pageOutput .=form_hidden('ID', $Contact->ID);
							$pageOutput .=form_hidden('InvoiceNumber', $invoiceTotals->InvoiceNumber);
							$pageOutput .= "			".form_label($Originator." Billing Guidelines")."\n";
							$pageOutput .= "			".form_textarea('ContactBillingGuidelines',$Contact->ContactBillingGuidelines,'class="ClientBillingGuidelines form-control" style="min-width: 100%"')."\n";
						...
						$pageOutput.='<a href="'.site_url().'contacts/zt2016_contact_info/'.$Contact->ID.'">'.$Originator;
						if (!empty($Contact->ContactBillingGuidelines))
						{
							$pageOutput.=" **";
						}

...
class Zt2016_invoice_ogone_form extends MY_Controller {
	...
	public function index()
	{
		...
		if ($this->input->post('InvoiceNumber'))
		{
			 ...
			 if ($this->input->post('InvoiceTotal')) {
				...
				//exit();	
				redirect('invoicing/zt2016_viewinvoice'/$this->input->post('invoicenumber'), 'refresh');
			 }
		} 
	...
	function  _getinvoiceContent($invoiceTotals)
	{
		...
		if (strtolower($clientInfo -> Country)=="the netherlands") {
...
//			$existing_ogone_url.='						<input type="hidden" name="Status" value="'.$invoiceTotals->Status.'">'."\n";
	...
	function _get_ogone_form($invoiceTotals){
		...
		if (isset($invoiceTotals->PostInvoiceVAT)) {
			//$temptotal = number_format($invoiceTotals->InvoiceTotal + ($invoiceTotals->InvoiceTotal*.21),2,".","");
			$ogoneform.='							<input type="text" class="form-control" name="InvoiceTotal" id="InvoiceTotal" value="'.$invoiceTotals->PostInvoiceVAT.'">'."\n";
		} else {
			$ogoneform.='							<input type="text" class="form-control" name="InvoiceTotal" id="InvoiceTotal" value="'.$invoiceTotals->InvoiceTotal.'">'."\n";
		}
 		$ogoneform.='					</div>'."\n";
 		$ogoneform.='				</div>'."\n";
		...
		if (isset($invoiceTotals->PostInvoiceVAT)) {
 		...
 		$ogoneform.='					</div>'."\n";
  		$ogoneform.='				<div class="form-group ">'."\n";
 		$ogoneform.='					<input type="submit" name="ogonesubmit" value="Update HTML" class="btn btn-sm btn-success">'."\n";
 		$ogoneform.='				</div>'."\n";
		$ogoneform.="			</form>"."\n";
		$ogoneform.="		</div>"."\n";
	...
	function _get_ogone_panels($invoiceTotals){
		...
		$content.="				<form method=\"post\" action=\"".base_url()."invoicing/zt2016_post_ogone_ZOWsite"."\">"."\n";	
...
<html>
	...
	<body class="paymentsinvoice">
		<div id="site-wrapper">
   		...
   		<h1 class="section-intro">Invoice Number: '.$invoicenumber.' - '.number_format($invoicetotal,2,".",",").' EUR</h1>

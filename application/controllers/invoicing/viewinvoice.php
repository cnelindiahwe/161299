...
class Viewinvoice extends MY_Controller {
	...
	function index()
	{
		...
		$templateVars['pageOutput'] .=  $this->_gettopbar($invoicenumber,$clientName);
	...
	// ################## top ##################	
	function  _gettopbar($invoicenumber,$clientName)
	{
			$entries ="<div id='newjobbuttons' class='zowtrakui-topbar'>\n";
			$entries .="<h1>Invoice ".$invoicenumber."</h1>";
			$entries .=$this-> _ViewInvoiceForm($invoicenumber,$clientName);
			$clientName= str_replace(' ', '_', $clientName);
			//$clientName= str_replace(' ', '%20', $clientName);
			$clientName=str_replace("&","~",$clientName);
			$entries .="<a href=\"".site_url()."invoicing/csvpastinvoice/".$clientName."/".$invoicenumber."\" >Export</a>";
			$entries .="<a href=\"".site_url()."invoicing/invoiceogoneform/".$invoicenumber."\" >Generate Ogone Form</a>";
	...
	function  _getclient($invoicenumber)
	{
		...
		/*$longcode=substr($invoice, 2,1);
		if (ctype_alpha($longcode)){
			$clientcode=substr($invoice, 0,3);
		}
		else {
			$clientcode=substr($invoice, 0,2);
		...
		*/
		...
		$currentclientcode = $this->trakclients->GetEntry($options = array('ClientCode' => $clientcode));
		return $currentclientcode->CompanyName;

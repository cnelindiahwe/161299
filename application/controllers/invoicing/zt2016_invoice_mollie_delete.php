...
class Zt2016_invoice_mollie_delete extends MY_Controller {
	...
	function index()
	{
		...
		#Determine whether there is flashdata
	    if($this->session->flashdata('InvoiceNumber')){		
			$invoiceinfo['InvoiceNumber']=$this->session->flashdata('InvoiceNumber');
			
		} 
		
		#Determine whether form was submitted	
		else if ($this->input->post('InvoiceNumber'))
		{
			$invoiceinfo['InvoiceNumber']=$this->input->post('InvoiceNumber');
		} 
		
		#Die if no invoice number received
		else {
					die ("Cannot delete Mollie page due to missing Invoice Number.<br/>Please use the back button on your browser.");
		}
		...
		$this->session->set_flashdata('SuccessMessage', 'Invoice Mollie page deleted.');

		if($this->session->flashdata('InvoiceNumber')){		
			redirect('/invoicing/zt2016_view_invoice/'.$this->session->flashdata('InvoiceNumber'));
		}
	...
	// ################## delete Mollie page from ZOW website##################	

<?php
use Mpdf\Mpdf;
class Zt2016_pdf_existing_invoice extends MY_Controller {
	...
	function index()
	{
		...
		if(empty ( $clientdata)) {
			$Message='No Client data found for invoice '.$invoicenumber;						
		...
		if(empty ($invoicedata)) {
			$Message='No invoice data found for invoice '.$invoicenumber;						
			$this->session->set_flashdata('SuccessMessage',$Message);

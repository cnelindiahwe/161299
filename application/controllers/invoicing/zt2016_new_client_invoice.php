...
class Zt2016_new_client_invoice extends MY_Controller {
	...
	function  _display_new_client_invoice($InvoiceData,$clientInfo,$clientsTable, $safeclientName,$StartDate,$EndDate,$excludelist=array())
	{
		...
		$invoice_header.= "<p class='top-buffer-10'>";
			...
			########## Invoices button
		 	$invoice_header.='<a href="'.site_url().'invoicing/zt2016_client_invoices/'.$safeclientName.'" class="btn btn-primary btn-xs ">Invoices</a>';
	...
	function _create_new_invoice_table($InvoiceData,$ClientInfo,$clientsTable, $safeclientName, $StartDate,$EndDate,$ExcludeList=array()){
		...
		$entries .= "</p>";
		$entries .= "<div>\n";
		$entries .= "	<table id=\"new-invoice-entries\" class=\"table table-striped table-condensed responsive  dtr-inline \" style=\"width: 100%; !important\">\n"; //dataTable
 
		$entries .="		<thead><tr><th class=\"no-sort\">Code</th><th  data-sortable=\"true\">Date</th><th data-sortable=\"true\">Originator</th><th  class=\"no-sort\">New</th><th class=\"no-sort\">Edits</th><th  class=\"no-sort\">Hours</th><th  class=\"no-sort\">File Name</th><th  class=\"no-sort\">Team</th><th id=\"exclude-header\" style=\"width:4em\"  class=\"no-sort \">Exc.</th></tr>";
	//	$entries .="		<tfoot><tr><th data-sortable=\"true\">Code</th><th data-sortable=\"true\">Date</th><th data-sortable=\"true\">Originator</th><th data-sortable=\"true\">New</th><th data-sortable=\"true\">Edits</th><th data-sortable=\"true\">Hours</th><th data-sortable=\"true\">File Name</th><th data-sortable=\"true\">Team</th><th>Trash</th><th id=\"exclude-header\">Exclude</th></tr></tfoot>\n";		
		...
		foreach($InvoiceData as $project)
		{
			if (!in_array($project->id, $ExcludeList)) {
				...
				$entries .= "<td class=\"date\" data-order=\"".strtotime($project->DateOut)."\">".$mysqldate. "</td>";
		...
		$Invoicecreate = $this->_generate_invoice_number($ClientInfo, $EndDate);
		
		$InvoiceTotals['Invoice_Number'] = $Invoicecreate['InvoiceNumber'];
		$InvoiceTotals['Invoice_Start'] = $Invoicecreate['InvoiceStart'];
		
		$InvoiceAllNotes=$this->_compile_invoice_notes($ClientInfo, $InvoiceTotals,$InvoiceData);
		...
		$entries=$invoiceform.$originator_box.$entries.$InvoiceAllNotes;
	...
	function _create_new_invoice_dates_form($InvoiceTotals,$ClientInfo,$InvoiceData, $StartDate,$EndDate,$ExcludeList=array()){
		...
		$new_invoice_form.='<div class="row"><div class="col-sm-4">'."\n";
		...
		$new_invoice_form.='<!--col-->'."\n";
		
		$new_invoice_form.='			</form></div>'."\n";
		...
		$new_invoice_form.=number_format($InvoiceTotals['Invoice_Cash_Total'], 2, '.', '' );
	...
	function  _New_Invoice_Create_Form($ClientInfo,$invoicedata,$InvoiceTotals,$StartDate,$EndDate,$excludelist=array())
	{
		...
		$hidden = array('Client' => $ClientInfo->CompanyName,
						...
						'StartDate' => $StartDate,								
						'EndDate' => $EndDate,								
						'InvoiceNumberStart' => $InvoiceTotals['Invoice_Start']							
						);
		...
		$ndata = array('name' => 'submit','value' => 'Create','class' => 'submitButton btn btn-success btn-b mb-3');
	...
	// ################## Generate client selector ##################	
		...
		$client_selector=form_open(site_url().$FormURL,$attributes);
	 	$client_selector.='				<div class="form-group pb-3">'."\n";
	...
	###++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++###	
	// Compile Invoice Notes
	###++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++###	

	/**
	 * _compile_invoice_notes
	 * Displays all relevant billing notes
	 */
	
	function _compile_invoice_notes($ClientInfo, $InvoiceTotals,$InvoiceData){
	
		$InvoiceAllNotes ="";
		$Originators="";                                                                                                                                                            

		##########  Notes row
		$InvoiceAllNotes.='	<div class="row" style="padding:0;">'."\n";
		$InvoiceAllNotes.='<hr style="margin:0.5em 0;"/>';
		//$InvoiceAllNotes.='		<div class="col-md-12"><h4>Notes</h4></div>';

			######### Client notes		
			$InvoiceAllNotes.='		<div class="col-md-4">'."\n";
			
				$attributes='id="client-billing-guidelines-form"';
				$formurl=site_url().'clients/zt2016_client_billing_info_update/';
				$InvoiceAllNotes.=form_open($formurl,$attributes )."\n";
				$InvoiceAllNotes .=form_hidden('ID', $ClientInfo->ID);
			 	$InvoiceAllNotes .= "			".form_label("Client Billing Guidelines")."\n";
				$InvoiceAllNotes .= "			".form_textarea('BillingGuidelines',$ClientInfo->BillingGuidelines,'id="ClientBillingGuidelines" class="form-control" style="min-width: 100%"')."\n";
				$ndata = array('class' => 'Notes-Submit-Button btn btn-sm','value' => 'Update Client Guidelines');
				$InvoiceAllNotes .= "<p>".form_submit($ndata)."</p>\n";	
				$InvoiceAllNotes.=form_close("\n");

	 		$InvoiceAllNotes.='		</div><!--col-->'."\n";	
			...
			$contactInfoTable = $this->zt2016_contacts_model->GetContact($options = array('CompanyName' =>$ClientInfo->CompanyName));		
			...
			foreach ($OriginatorTable as $Originator){
				...
				if (!empty($Originator->ContactBillingGuidelines))
					{$attributes='class="contact-billing-guidelines-form"';
						...
						$InvoiceAllNotes.=form_open($formurl,$attributes )."\n";
						$InvoiceAllNotes .=form_hidden('ID', $Originator->ID);
						$InvoiceAllNotes .= "			".form_label($OriginatorFullName." Billing Guidelines")."\n";
						$InvoiceAllNotes .= "			".form_textarea('ContactBillingGuidelines',$Originator->ContactBillingGuidelines,'class="ClientBillingGuidelines form-control" style="min-width: 100%"')."\n";
		...
		$InvoiceAllNotes.='	</div><!--row--> '."\n";
				
		

		return $InvoiceAllNotes;
	}
	...
	function _generate_invoice_number($ClientInfo, $EndDate){
		...
		// $InvoinceCount = $queryResult[0]->InvoiceNumberStart;
		// $num2 = "1";
		// // Convert the strings to integers and then add them
		// $sum = (int)$InvoinceCount + (int)$num2;

		// // Convert the result back to a string with leading zeros if needed
		// $invoicenumberCreate = str_pad($sum, strlen($InvoinceCount), '0', STR_PAD_LEFT);
		// $TestInvoiceNumber = 'INV-'.date("y", strtotime($EndDate)).'-'.$invoicenumberCreate.'-'.$InvoiceNumber;
		 
		$this->db->select('InvoiceNumberStart');
		$this->db->from('zowtrakglobalsetting');
		//$this->db->limit(1); 
		$query = $this->db->get();

		$start_invoice = $query->result();
			
		$InvoiceCount=$start_invoice[0]->InvoiceNumberStart;
		$finished = false;
		...
		{ 
			...
			if ($InvoiceCount<1000) {
				$TestInvoiceNumber = 'INV-'.date("y", strtotime($EndDate)).'-0'.$InvoiceCount.'-'.$InvoiceNumber;
			}
			else{
				$TestInvoiceNumber = 'INV-'.date("y", strtotime($EndDate)).'-'.$InvoiceCount.'-'.$InvoiceNumber;
			}
			$this->db->select('InvoiceNumber');
			$this->db->from('zowtrakinvoices');
			//$this->db->limit(1);
		...
		$InvoiceNumberhwe['InvoiceNumber']=$TestInvoiceNumber;
		$InvoiceNumberhwe['InvoiceStart']=$InvoiceCount;

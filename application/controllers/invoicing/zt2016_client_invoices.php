...
class Zt2016_client_invoices extends MY_Controller {
	...
	function  _display_existing_client_invoices($clientInvoicesTable,$clientInfo,$clientsTable,$safeclientName)
	{
		...
		############## panel header	
		$pageOutput.='<div class="panel panel-default" style="margin-top: 2em;""><div class="panel-heading">'."\n"; 
		
		if (!$clientInvoicesTable){
			$invoicecount=0;
		} else{
			$invoicecount=count ($clientInvoicesTable);
		}
		
		$pageOutput.='<h3 class="panel-title">'.$clientInfo->CompanyName.' : '.$invoicecount." existing invoices (all time)</h3>";
	...
	function _get_existing_client_invoices_table($clientInvoicesTable,$clientInfo) {
		...
		if (empty($clientInvoicesTable)) {
			...
			$InvoiceTableRaw.="<tr><th data-sortable=\"true\">Invoice Number</th><th data-sortable=\"true\">Billed</th><th data-sortable=\"true\">Due</th><th data-sortable=\"true\">Paid</th><th data-sortable=\"true\">Amount (".$clientInfo->Currency.")</th><th data-sortable=\"true\">Status</th><th data-sortable=\"true\">Originators</th></tr>"."\n";
			$InvoiceTableRaw.="</thead>"."\n";
			$InvoiceTableRaw.="<tfoot>"."\n";
			$InvoiceTableRaw.="<tr><th >Invoice Number</th><th>Billed</th><th>Due</th><th class=\"text-right\">Amount</th><th>Paid</th><th>Status</th><th></th></tr>"."\n";
			$InvoiceTableRaw.="</tfoot>"."\n";		
			...
			foreach ($clientInvoicesTable as $row){
				//$InvoiceTableRaw.= "<tr><td><a href=\"".site_url()."contacts/contacts_profile/".$row->ID."\">".$row->FirstName. " ". $row->LastName."</a></td><td>".$row->CompanyName."</td><td>".date('Y', strtotime($row->FirstContactIteration))."</td></tr>" ."\n";
				 ...
				 if ($row->Status=="BILLED"){
					...
					$InvoiceTableRaw.= "<tr class=\"info paid_invoice\">";
				 	...
				 	$InvoiceStatus = $row->Status;				
					$InvoiceTableRaw.= "<tr class=\"danger disputed_invoice\">";
				 }			
				...
				if ($row->Status=="BILLED"){
					...
					$InvoiceTableRaw.=date('j-M-Y', strtotime($row->PaidDate));
				}
				$InvoiceTableRaw.="</td>";

...
class Reportsnew4 extends MY_Controller {

	function Reportsnew4()
	{
	...
	function  _monthData($options=array())
	{
			...
			$EndDate = date( '2011-11-30');

	$data="<h2>Contact Totals</h2>";
	
	$data.="<h4>EUR DTP</h4>";
	$allcontacts=$this->trakreportsnew->ContactsByDate($options=array('StartDate'=>$StartDate,'EndDate'=>$EndDate,'WorkType'=>'Office','Currency'=>'EUR'));	
	$data.="<table>";	
	


	foreach ($allcontacts as $thiscontact)
	{
		$this->db->select_min('DateIn');		
		$this->db->from('zowtrakentries'); 
		$this->db->where('Originator',$thiscontact->Originator);
		$query=$this->db->get();
		$contact=$query->row(0);
		

		
	if ($contact->DateIn>$StartDate)
		{
		$data.="<tr><td>".$thiscontact->Originator."</td><td>";
		$data.="<td><td>".$contact->DateIn."</td><td>";

		$company=$this->trakreportsnew->_getcontactcompany($thiscontact->Originator);
		$data.="<td><td>".$company."</td><td>";
		$cashtotal=$this->trakreportsnew->_contactcashtotal($options=array ('StartDate'=>$StartDate, 'EndDate'=>$EndDate, 'Originator'=>$thiscontact->Originator));

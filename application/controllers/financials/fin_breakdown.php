...
class Fin_breakdown extends MY_Controller {
	...
	function index()
	{
		...
		array_push($admittedyears, "All Time");
 ...
 */
 	...
 	function _yearclienttotals($options=array()){
		...
		if ($year== $current_year || $year== "All Time") {
...
//				$ClientTableRaw  = $this->trakclients->GetEntry($options = array('Trash'=>'0','sortBy'=>'CompanyName','sortDirection'=>'ASC	'));
				foreach ($ClientTableRaw as $row){
				  ...
				  //Get unbilled entry totals from db
				...
				//Get billed entry totals from db
				
				  $this->db->select_sum('InvoiceEntryTotal','InvoiceRevenue');
				  $this->db->select_sum('InvoiceTime','Hours');
				  $this->db->where('Client',$row->CompanyName);
				  $where = "(Status = 'BILLED' OR Status = 'PAID')";
				  $this->db->where($where );
				  $this->db->where('DateOut >=', $StartDate);
				  $this->db->where('DateOut <= ', $EndDate);
				   $this->db->where('Trash =',0);
				  ...
				  if ($htotal!=0 || $query2->row()->InvoiceRevenue!=0  ){
					 ...
					 $invoicerevenue+=$query2->row()->InvoiceRevenue;
					 //add to totals
					 ...
					 $clienthours.=",". round($htotal+$query2->row()->Hours,1);
				}
			...
			//$this->db->set('ClientHours', $clienthours);	
			if ($existflag==1) {
				$this->db->where('Year',$StartDate);
 ...
 *
 * Adds timeframe drop down to top bar 
 *
 * @access	public
 * @return	string
 */

			...
			foreach($StatusType as $Status) {
				...
				foreach ($ClientList as $clientb) {
					...
					if($t == 1){
							...
							$AllTotals[$clientb->Client][$Status][ucfirst($name->fname)]['cashtotal']=$bookedtotal*$monthtotals[$clientb->Client]['Price'];
							$AllTotals[$clientb->Client][$Status][ucfirst($name->fname)]['total']=$bookedtotal;
							$AllTotals[$clientb->Client][$Status][ucfirst($name->fname)]['span']="(".$row['NewSlides']." N, ".$row['EditedSlides']." E, ".$row['Hours']." H)";
				   ...
				   $AllTotals[$clientb->Client][$Status][ucfirst($name->fname)]['cashtotal']=$bookedtotal*$monthtotals[$clientb->Client]['Price'];
				   $AllTotals[$clientb->Client][$Status][ucfirst($name->fname)]['total']=$bookedtotal;
				   $AllTotals[$clientb->Client][$Status][ucfirst($name->fname)]['span']="(".$row_new_hwe." N, ".$row_edit_hwe." E, ".$row_Hours_hwe." H)";
			   		}else{
							$subtotalbooked= $row['EditedSlides']*$clientdata->PriceEdits;
							//Add slides and divide by slides per hour
							$subtotalbooked= $row['NewSlides']+$subtotalbooked;

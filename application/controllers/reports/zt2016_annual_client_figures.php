...
class Zt2016_annual_client_figures extends MY_Controller {
	...
	function index()
	{
		...
		$this->load->model('zt2016_client_figures_model', '', TRUE);
		...
		#### Title 
			...
			### buttons
				...
				//<a href="'.site_url().'reports/zt2016_annual_originator_figures/'.$ClientInfo->SafeclientName.'" class="btn btn-danger btn-sm pull-right">Client Info</a>';	
					$page_content.= "<p style='position:relative; top:-.65em;'>";
				# Client Info button	
	...
	function   _display_client_data($ClientInfo)
	{
		...
		$page_content.= "<thead><tr><th>Year</th><th>Billed Hours</th><th>Jobs</th><th>Originators</th><th>Avg. Hours per Job</th><th>Avg. Hours per Originator</th><th>Avg. Jobs per Originator</th></tr></thead>";	
			...
			foreach ($period as $dt) {
				...
				$row= $this->_get_row_data($options = array('Client' => $ClientInfo, 'CurrentYear'=>$dt->format('Y')));
				...
				if ($row['BilledHours']>0){
					...
					$page_content.= "<td>".number_format($row['BilledHours']/$row['Originators'], 1)." </td>";
...
// ################## row data  ##################	
	function   _get_row_data($options =  array())
	{
		...
		# data from 2 years ago anbd before is read from existing table
		if ($options['CurrentYear'] <  date("Y")-2){			
			$row= $this->zt2016_client_figures_model->GetClientAnnualData($options2 = array('CompanyName' => $options['Client']->CompanyName, 'Year'=>$options['CurrentYear']));

			#if no data exists, calculate and insert data in dbd
			if(empty($row)){

				$row= $this->zt2016_reports_model->_Client_Annual_Figures_data($options = array('Client' => $options['Client'], 'CurrentYear'=>$options['CurrentYear']));
				
				//insert values into ClientAnnualData table
				$insertedrow = $this->zt2016_client_figures_model->AddClientAnnualData($options = array('CompanyName' =>  $options['Client']->CompanyName, 'Year'=>$row['Year'],"BilledHours"=>$row['BilledHours'], "Jobs"=>$row['Jobs'], "Originators"=>$row['Originators'], "NewSlides"=>$row['NewSlides'], "EditedSlides"=>$row['EditedSlides'], "AdditionalHours"=>$row['Additionalhours']));
		...
		# data for the last 2 years is calculated and (re)inserted in existing table
			...
			//insert values into ClientAnnualData table
			$CheckAnnualClient = $this->zt2016_client_figures_model->GetClientAnnualData($options2 = array('CompanyName' => $options['Client']->CompanyName, 'Year'=>$options['CurrentYear']));
			
			if($CheckAnnualClient) {
				$insertedrow = $this->zt2016_client_figures_model->UpdateClientAnnualData($options = array('ID' => $CheckAnnualClient->ID, 'CompanyName' =>  $options['Client']->CompanyName, 'Year'=>$row['Year'], "BilledHours"=>$row['BilledHours'], "Jobs"=>$row['Jobs'], "Originators"=>$row['Originators'], "NewSlides"=>$row['NewSlides'], "EditedSlides"=>$row['EditedSlides'], "AdditionalHours"=>$row['Additionalhours']));
			} else{				
				$insertedrow = $this->zt2016_client_figures_model->AddClientAnnualData($options = array('CompanyName' =>  $options['Client']->CompanyName, 'Year'=>$row['Year'],"BilledHours"=>$row['BilledHours'], "Jobs"=>$row['Jobs'], "Originators"=>$row['Originators'], "NewSlides"=>$row['NewSlides'], "EditedSlides"=>$row['EditedSlides'], "AdditionalHours"=>$row['Additionalhours']));
			}

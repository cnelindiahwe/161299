...
class Newsplit extends MY_Controller {
	...
	function  _getMonthSplits	($ClientList,$StartDate,$EndDate)
	{
		...
		//}
		//else {
			$EndMonth = date( 'Y-m-t',strtotime($EndDate));
				...
				foreach($Status as $StatusType)
					...
					foreach($Clientlist as $clientb)
						...
						if ($querybooked) {  
						  ...
						  $query2 = $this->trakclients->GetEntry($options = array('CompanyName' => $clientb->Client));
							//echo $this->db->last_query();
						/*	echo $clientb->Client;
							if ($query2->PriceEdits)
							{

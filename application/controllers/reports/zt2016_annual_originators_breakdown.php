...
class Zt2016_annual_originators_breakdown extends MY_Controller {
	...
	function index()
	{
		...
		$templateData['title'] = $report_year.' Originators Breakdown - ZOWTrak';
	...
	function _create_page($report_year,$ZOWuser)
	{
		...
		$page_content.='<div class="panel-heading">'."\n";
			...
			### Month Tile and selector
				...
				$year_selector_info['FormURL']=site_url()."reports/zt2016_annual_originators_breakdown";
				...
				$page_content.=	'<div class="pull-left" style="margin-right:1rem; padding-top:.7rem;">Annual Originators Breakdown for</div>'."\n";
				$page_content.=year_selector($report_year,$year_selector_info)."\n";		
				########## left buttons
				##### monthyl client breakdown button
				$page_content.='<a href="'.site_url().'reports/zt2016_annual_clients_breakdown/'.$report_year.'" class="btn btn-primary btn-sm">Client Data</a>';		
				###  monthly originators breakdown button
				$page_content.='<a href="'.site_url().'reports/zt2016_monthly_originators_breakdown/Jan%20'.$report_year.'" class="btn btn-info btn-sm">Monthly Data</a>';		
		...
		$page_content.='<div class="panel-body">'."\n";
			...
			$page_content.=$this->_year_Data($options);
...
// ################## General month numbers	
	...
	function  _year_Data($options=array())
	{
	...
	function _create_month_table($options)
	{
				...
				if ($numjobs >0) {
					...
					foreach ($options['monthtotal'] as $row) {
						...
						//echo $row['Originator']."".$row['Client']." ".$OriginatorData->ID."<br/>";

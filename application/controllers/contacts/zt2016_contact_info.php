...
// ################## create page ##################	
	function  _display_page($ContactInfo,$ContactsTable,$ClientsTable,$ClientInfo,$ZOWuser)
	{
		...
		## clients dropdown
		$page_content .='<div class="row"><div class="col-sm-4">';
		$page_content.= _display_clients_control($ClientsTable,$ClientInfo,$FormURL);
		## contacts dropdown
		$page_content .='</div><div class="col-sm-4">';
		...
		$page_content.='<div class="panel-heading">'."\n";
			...
			$page_content.= '<div class="button_group_hwe" ><form action="'.$formurl.'" '.$attributes.' method="post" accept-charset="utf-8" style="display:inline;">';
			...
			if ($ZOWuser=="miguel" ||$ZOWuser=="sunil.singal"  || $ZOWuser=="jirka.blom" ) {
	 			$page_content.='<a href="'.site_url().'contacts/zt2016_contact_edit/'.$ContactInfo->ID.'" class="btn btn-success btn-sm  pull-right">Edit</a>';
	  		}

		########## Duplicate contact
			
			$attributes='class="form-inline pull-right" id="duplicatecontactform" name="duplicatecontactform"';
			$formurl=site_url().'contacts/zt2016_contact_duplicate';
		
			$page_content.= '<form action="'.$formurl.'" '.$attributes.' method="post" accept-charset="utf-8" style="display:inline;">';
			$page_content.='	<div class="form-group">'."\n";
			$page_content.='		<input type="hidden" id="DuplicateCurrent_Contact" name="Current_Contact" value="'.$ContactInfo->ID.'">'."\n";
			$page_content.='		<input type="submit" id="Duplicate_contact_submit" name="Duplicate_contact_submit" value="Duplicate Contact" class="btn btn-warning btn-sm " >'."\n";
			$page_content.='	</div>'."\n";
			$page_content.="</form>"."\n";			
		...
		$page_content.='<div class="panel-body">'."\n";
			...
			########## col 1	
				...
				$page_content.='				<div class="col-sm-4">'."\n";
					...
					$page_content.=$this->_display_contact_information($ContactInfo,$ClientInfo);
			...
			########## col 3	
				$page_content.='				<div class="col-sm-4">'."\n";
					...
					########## tracking info
					$page_content.=$this->_display_history_information($ContactInfo,$ClientInfo);
...
// ################## display location information ##################	
	function   _display_location_information ($ContactInfo,$ClientInfo)
	{
...
// ################## display history information ##################	
	function   _display_history_information ($ContactInfo,$ClientInfo)
	{
		
		########## headers
		$page_content='<h5  class="text-uppercase text-primary"">History information</h5>'."\n";
		$page_content.='<table class="table">'."\n";
